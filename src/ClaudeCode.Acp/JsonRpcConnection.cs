...
{
    /// <summary>A `session/update`-style one-way message: <c>{ jsonrpc, method, params }</c> with no `id`.</summary>
    ...
    /// <summary>Handles one inbound JSON-RPC *request* (agent -&gt; client) and returns its `result` payload,
    /// or throws <see cref="AcpRemoteException"/>/any exception to have it reported back as a JSON-RPC error.</summary>
    internal delegate Task<JsonNode?> JsonRpcRequestHandler(string method, JsonNode? @params, CancellationToken cancellationToken);

    /// <summary>
    /// Minimal JSON-RPC 2.0 transport over newline-delimited JSON (NDJSON) carried by two independent
    /// streams (read side / write side) - deliberately not tied to <see cref="System.Diagnostics.Process"/> so it
    /// can be driven directly against in-memory streams in tests.
    ///
    /// Owns: outbound request/response correlation (<see cref="SendRequestAsync"/> via a
    /// ConcurrentDictionary&lt;id, TaskCompletionSource&gt;), a background pump task reading the input stream
    /// line by line, serialized writes (a semaphore guarantees one complete JSON line is written at a time so
    /// concurrent <see cref="SendRequestAsync"/>/<see cref="SendNotificationAsync"/>/response-writes never interleave),
    /// and dispatch of inbound requests/notifications to <see cref="RequestHandler"/> / <see cref="NotificationReceived"/>.
    /// </summary>
    internal sealed class JsonRpcConnection : IAsyncDisposable
    {
        ...
        /// <summary>Invoked for every inbound JSON-RPC request. Must be set before <see cref="Start"/> receives one.</summary>
        public JsonRpcRequestHandler? RequestHandler { get; set; }

        public event EventHandler<JsonRpcNotification>? NotificationReceived;

        /// <summary>Raised once, when the read pump stops (peer closed the stream, or an unrecoverable read/parse
        /// error occurred). The exception is null for a clean EOF shutdown.</summary>
        ...
        /// <summary>Closes only the outbound stream, signalling EOF to the remote peer's stdin without
        /// interrupting the inbound read pump. Used for graceful shutdown (close stdin, let the agent exit on
        /// its own, then <see cref="DisposeAsync"/>/kill if it doesn't).</summary>
        ...
        /// <summary>Captures a JSON-RPC `id` by value (long or string) so it can be echoed back in a new,
        /// unparented <see cref="JsonNode"/> - a <see cref="JsonNode"/> already attached to a parsed document
        /// cannot be re-attached to a new <see cref="JsonObject"/> without throwing.</summary>
        private readonly struct WireId
        {
            ...
            public JsonNode ToNode() => _text != null ? JsonValue.Create(_text)! : JsonValue.Create(_numeric)!;
        }
    }
}
